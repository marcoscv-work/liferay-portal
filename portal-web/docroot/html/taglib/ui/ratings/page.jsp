						...
						<liferay-util:whitespace-remover>
							...
							<c:choose>
								<c:when test="<%= !themeDisplay.isSignedIn() || TrashUtil.isInTrash(className, classPK) %>">
									<span class="icon-monospaced glyphicon glyphicon-thumbs-up rating-element rating-thumb-up rating-<%= (yourScore > 0) ? "on" : "off" %>" title="<liferay-ui:message key="ratings-are-disabled-because-this-entry-is-in-the-recycle-bin" />">
									</span>
									<span><%= positiveVotes %></span>

									<c:if test="<%= type.equals(RatingsType.THUMBS.getValue()) %>">
										<span class="icon-monospaced glyphicon glyphicon-thumbs-down rating-element rating-thumb-down rating-<%= (yourScore == 0) ? "on" : "off" %>" title="<liferay-ui:message key="ratings-are-disabled-because-this-entry-is-in-the-recycle-bin" />">
										</span>
								...
								<c:otherwise>
									<a href="javascript:;">
										<i class="icon-monospaced glyphicon glyphicon-thumbs-up rating-element rating-thumb-up rating-<%= (yourScore > 0) ? "on" : "off" %>"></i>
										<span><%= positiveVotes %></span>
									</a>

									<c:if test="<%= type.equals(RatingsType.THUMBS.getValue()) %>">
										<a href="javascript:;">
											<i class="icon-monospaced glyphicon glyphicon-thumbs-down rating-element rating-thumb-down rating-<%= (yourScore == 0) ? "on" : "off" %>"></i>
											<span><%= negativeVotes %></span>
										</a>
									</c:if>
